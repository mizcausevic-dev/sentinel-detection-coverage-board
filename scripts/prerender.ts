...
import {
  ...
  summary,
  verification
} from "../src/services/sentinelDetectionCoverageBoardService.js";
import {
  renderCoverageGaps,
  renderDetectionLane,
  renderDocs,
...
const pages: Record<string, string> = {
  ...
  [path.join("detection-lane", "index.html")]: renderDetectionLane(),
  [path.join("coverage-gaps", "index.html")]: renderCoverageGaps(),
  [path.join("incident-posture", "index.html")]: renderIncidentPosture(),
...
const apiPayloads: Record<string, unknown> = {
  ...
  [path.join("api", "detection-lane.json")]: detectionLane(),
  [path.join("api", "coverage-gaps.json")]: coverageGaps(),
  [path.join("api", "incident-posture.json")]: incidentPosture(),
